...
public class URLImporterTest {
    ...
    public void convertToObject() throws Exception {
        ...
        //Act
        CalendarEvent calendarEvent = new CalendarEvent("example1", new Date(Date.parse("2016/01/22 00:00")),
                new Date(Date.parse("2016/01/23 00:00")), "OPIS1", "Tag", TimeZone.getTimeZone("Europe/Warsaw"));

        //Assert
        assertEquals(calendarEvent, urlImporter.convertToObject().get(0));
        assertEquals(calendarEvent, urlImporter.convertToObject().get(1));
    }
}